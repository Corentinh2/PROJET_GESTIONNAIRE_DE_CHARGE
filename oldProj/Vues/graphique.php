                ...
                <?php if (isset($_GET['plage']) && $_GET['plage'] === 'semaine'): ?>
                    <div class="selecteur">
                        ...
                        <input type="date" id="date_debut" 
                               value="<?= isset($_GET['date_debut']) ? $_GET['date_debut'] : date('Y-m-d', strtotime('monday this week')) ?>"
                               style="background:#0f0f0f; border:1px solid #2a2a2a; color:white; padding:8px 12px; border-radius:6px; cursor:pointer;">
                        <label>Au :</label>
                        <input type="date" id="date_fin"
                               value="<?= isset($_GET['date_fin']) ? $_GET['date_fin'] : date('Y-m-d', strtotime('sunday this week')) ?>"
                               style="background:#0f0f0f; border:1px solid #2a2a2a; color:white; padding:8px 12px; border-radius:6px; cursor:pointer;">
                        <button onclick="appliquerDates()" 
                                style="padding:8px 16px; background:#00d4ff; color:#0f0f0f; border:none; border-radius:6px; cursor:pointer; font-weight:bold;">
                            Appliquer
                        </button>
                    </div>
                    <script>
                        function appliquerDates() {
                            ...
                            window.location.href = '/index.php?action=EnregistreGraph&id=<?= $_GET['id'] ?>&plage=semaine&date_debut=' + debut + '&date_fin=' + fin;
                        }
                    </script>
                <?php endif; ?>
